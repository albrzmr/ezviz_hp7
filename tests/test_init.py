"""Smoke tests for the integration package itself.

These don't load Home Assistant — they verify that the manifest and
package layout are well-formed (catches typos, missing version bumps,
unsynced ``hacs.json`` / ``manifest.json`` etc.).  Heavier
HA-fixture-based tests can live in their own ``test_*.py`` files
once ``pytest-homeassistant-custom-component`` plumbing is in.
"""
